...
In this assignment, we learn how to inherit classes from each other using the juvenile and parent classes with books
'''
...
class AdultPatron(LibraryPatron):
    def __init__(self, name):
        super().__init__(name)
        self.bookCheckOutLimit = 4

    def checkOutBook(self, book):
        super().checkOutBook(self.bookCheckOutLimit, book[0])
...
class JuvenilePatron(LibraryPatron):
    ...
    def checkOutBook(self, book):
        if book[1] == 'Juvenile':
            ...
            print("Sorry", book[0], "is an adult book")
